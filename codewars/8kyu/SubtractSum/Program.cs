...
int number = 325;
Console.WriteLine(SubtractSum(number));

string SubtractSum(int number)
{
    int sub = SubtractDigitSum(number);

    while (!fruits.ContainsKey(sub))
    {
        sub = SubtractDigitSum(sub);
    }

    return fruits[sub];
}

static int SubtractDigitSum(int number)
{
    int sum = 0;
    int n = number;
